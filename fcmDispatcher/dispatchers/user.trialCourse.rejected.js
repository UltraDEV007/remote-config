...
exports.dispatch = async ({ payload }, { ctxData, utils, helpers }) => {
  ...
  const title = i18n.t('RemoteConfig.TrialCourse.UserTrialCourseReject.title');

  // const body = `${userDisplayName} huỷ đăng ký khoá học ${courseDisplayName}.`;

  const body = i18n.t('RemoteConfig.TrialCourse.UserTrialCourseReject.body', {
  ...
  return {
    ...
    data: {
      type: 'user.trialCourse.rejected',
      sound: 'sound1',
    },
    apns: {
      payload: {
