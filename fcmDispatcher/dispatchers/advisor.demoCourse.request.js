...
exports.dispatch = async ({ payload }, { ctxData, utils, helpers }) => {
  ...
  const title = i18n.t('RemoteConfig.DemoCourse.AdvisorDemoCourseRequest.title');

  // const body = `${userDisplayName} huỷ đăng ký khoá học ${courseDisplayName}.`;

  const body = i18n.t('RemoteConfig.DemoCourse.AdvisorDemoCourseRequest.body', {
  ...
  return {
    ...
    data: {
      type: 'advisor.demoCourse.request',
      sound: 'sound1',
    },
    apns: {
      payload: {
    ...
    blocks: [
      {
        ...
        text: {
          type: 'plain_text',
          text: 'advisor.demoCourse.request',
        },
      },
      {
        type: 'context',
